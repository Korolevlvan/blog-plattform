...
def add_comment(slug: str, comment: CommentCreate, current_user: User = Depends(get_current_user_id), db: Session = Depends(get_db)):
    article = get_article_by_slug(slug, db)
    db_comment = Comment(body=comment.body, article_id=article.id, user_id=current_user_id)
    db.add(db_comment)
    db.commit()
...
def delete_comment(slug: str, comment_id: int, current_user: User = Depends(get_current_user_id), db: Session = Depends(get_db)):
    article = get_article_by_slug(slug, db)
